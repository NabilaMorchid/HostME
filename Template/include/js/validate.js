$(document).ready(function () {
    ...
    /*in case the user choose change local temporary*/
    $('select#categorie').change(function(){
        var val = $(this).val();
        if(val === '1' || val ===''){
            $('div#tempSelect , div#tempCin , div#tempCout ').hide();
            $('div#prx').show();
        }else if(val === '2'){
            $('div#prx').hide();
            $('div#tempSelect , div#tempCin , div#tempCout').show();
        }
    });
    ...
    $('form#SignUp input[type="text"] , form#SignUp input[type="password"] , form#logIn input[type="text"] , form#logIn input[type="password"] , form#creatPub input[type="text"] , form#creatPub input[type="password"] , form#creatPub textarea ').tooltipster({
        trigger: 'custom',
        onlyOne: false,
        position: 'right'
    });
    // this function is to accept only characters
    jQuery.validator.addMethod("accept", function(value, element, param) {
        return value.match(new RegExp("^" + param + "$"));
    },'please enter only alphabetic characters');
    /*this function is reponsible on validating date on american format*/
    jQuery.validator.addMethod("americanDate", function(value, element) {
        return value.match(/^\d\d?\-\d\d?\-\d\d\d\d$/);
    },"Please enter a date in the format dd-mm-yyyy.");
    // initialize validate plugin on the form
    $('form#SignUp , form#logIn ').validate({
    ...
    //validate creat pub form
    $('form#creatPub').validate({
        errorPlacement: function (error, element) {
            $(element).tooltipster('update', $(error).text());
            $(element).tooltipster('show');
        },
